import React from "react";

interface IconProps {
  color?: string; // Optional, to allow color customization if needed
  size?: number;  // Optional, to allow size customization if needed
}

const Vault: React.FC<IconProps> = ({
  color = "#FFEC6E", // Default color, you can remove this if not needed
  size = 44,        // Fixed size
}) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width={size}
    height={size}
    role="img"
    aria-labelledby="VaultTitle"
  >
    <title id="VaultTitle">Vault</title>
    <path
        d="M0 0l11.955 24L24 0zm13.366 4.827h1.393v1.38h-1.393zm-2.77 5.569H9.22V8.993h1.389zm0-2.087H9.22V6.906h1.389zm0-2.086H9.22V4.819h1.389zm2.087 6.263h-1.377V11.08h1.388zm0-2.09h-1.377V8.993h1.388zm0-2.087h-1.377V6.906h1.388zm0-2.086h-1.377V4.819h1.388zm.683.683h1.393v1.389h-1.393zm0 3.475V8.993h1.389v1.388Z" />
</svg>
);

export default Vault;